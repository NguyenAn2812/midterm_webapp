...
const CommentsContainer = ({ videoId }) => {
  ...
  const formatTimeAgo = (dateStr) => {
    ...
    for (let [unit, value] of Object.entries(intervals)) {
      ...
      if (count > 0) return `${count} ${unit} trước`;
    }
  ...
  return (
    <div className="text-white">
      <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
        <Icon icon="mdi:comment-outline" className="text-xl" /> Bình luận
      </h2>
          ...
          <div key={c._id} className="flex items-start gap-3 border-b border-[#303030] pb-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <p className="text-sm font-semibold">{c.user?.username || "Người dùng ẩn danh"}</p>
