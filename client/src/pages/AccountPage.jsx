...
const AccountPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    login(token); // cập nhật lại avatar trên navbar
    alert("Thông tin đã được cập nhật!");
    } catch (err) {
      alert("Lỗi cập nhật: " + err.response?.data?.message || "Unknown error");
    }
  };
...
/>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm mb-1">Mật khẩu mới (tùy chọn)</label>
          <input
            type="password"
            value={password}
        ...
        <div>
          <label className="block text-sm mb-1">Avatar mới</label>
          <input
            type="file"
            accept="image/*"
